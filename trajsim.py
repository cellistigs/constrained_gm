...
modelfolder = 'Video_ball_color_statvar_noise_small_ckpts/'
...
filenames = ['datadirectory/toydynamics_nograv/Video_ball_color_medtrain.tfrecords']
...
while True:
    try:
        input_ims = sess.run(next_images_it)
        gen_images,samples = sess.run([sampled_images,hidden_activations],feed_dict={inputs:input_ims})
        xmin = min(samples[:,0])
        xmax = max(samples[:,0])
        ymin = min(samples[:,1])
        ymax = max(samples[:,1])
